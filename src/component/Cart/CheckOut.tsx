...
const CheckOut = ({
    ...
    const handlerPlaceOrder = async () => {
        ...
        const checkStatus = await payToCart(dataPayToCart)

        if (checkStatus.error) {
            toastCustom(
                "error",
                "Insufficient quantity to process the transaction"
            )
        } else {
            toastCustom("success", "Order successfully")
            setOpen(false)
        }
    }
            ...
            <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
                ...
                <Box className="flex items-center justify-around my-10">
                    <Box className="flex items-center gap-[100px]">
                        <Typography>
                            Total
                            {`( ${productsCheckOut.length} products)`}:{price}$
                        </Typography>
                        <Button
